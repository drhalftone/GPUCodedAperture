...
void main()
{
    ...
    for (int col = 0; col < 8; col++){
        qt_fragColor += coefficientsA[2*col + iterB] * texelFetch(qt_textureA, ivec2((coord.x/4)*2 + iterA - 2*(col - 8), coord.y), 0);
        qt_fragColor += coefficientsB[2*col + iterB] * texelFetch(qt_textureB, ivec2((coord.x/4)*2 + iterA - 2*(col - 8), coord.y), 0);
    }
