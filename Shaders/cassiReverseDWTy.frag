...
void main()
{
    ...
    for (int row = 0; row < 8; row++){
        qt_fragColor += coefficientsA[2*row + iterB] * texelFetch(qt_textureA, ivec2(coord.x, coord.y/2 - row + 8) + ivec2(qt_positionA), 0);
        qt_fragColor += coefficientsB[2*row + iterB] * texelFetch(qt_textureB, ivec2(coord.x, coord.y/2 - row + 8) + ivec2(qt_positionB), 0);
    }
